...
from renweave.io import discard_intermediate
...
class CorePipelineTests(unittest.TestCase):
    ...
    def test_emitter_never_deletes_previous_rpy_artifacts(self) -> None:
        index = ProjectIndexer().build(self.root)
        translations = {unit.id: f"ZH: {unit.source}" for unit in index.text_units}
        output = Path(self.temp.name) / "retained-output"
        retained = output / "game" / "tl" / "zh_hans" / "previous-build.rpy"
        retained.parent.mkdir(parents=True)
        retained_payload = b"# Generated by RenWeave.\n# Previous completed artifact.\n"
        retained.write_bytes(retained_payload)

        RenpyTranslationEmitter().emit(index, translations, "zh_hans", output)

        self.assertEqual(retained.read_bytes(), retained_payload)

    def test_incremental_emitter_merges_dialogue_and_strings_into_original_file(self) -> None:
        index = ProjectIndexer().build(self.root)
        translations = {unit.id: f"ZH: {unit.source}" for unit in index.text_units}
        dialogue = next(unit for unit in index.text_units if unit.source == "We made it.")
        menu = next(unit for unit in index.text_units if unit.source == "Go outside")
        identifier = RenpyTranslationEmitter.dialogue_identifiers(index)[dialogue.id]
        existing = self.game / "tl" / "zh_hans" / "script.rpy"
        existing.write_bytes(
            b"\xef\xbb\xbf"
            + (
                "# User header\r\n"
                "# script.rpy:9\r\n"
                f"translate zh_hans {identifier}:\r\n\r\n"
                "    # eve \"Old source\"\r\n"
                "    eve \"\u65e7\u8bd1\u6587\"\r\n\r\n"
                "translate zh_hans strings:\r\n\r\n"
                "    old \"Go outside\"\r\n"
        ...
        output = Path(self.temp.name) / "merged-incremental-output"
        ...
        manifest = RenpyTranslationEmitter().emit(
            ...
            "zh_hans",
            output,
            existing_language_dir=self.game / "tl" / "zh_hans",
            reused_unit_ids={
                unit.id for unit in index.text_units if unit.id not in {dialogue.id, menu.id}
            },
        )

        merged = output / "game" / "tl" / "zh_hans" / "script.rpy"
        payload = merged.read_bytes()
        text = payload.decode("utf-8-sig")
        self.assertTrue(payload.startswith(b"\xef\xbb\xbf"))
        self.assertIn("\r\n", text)
        self.assertIn("# User header", text)
        self.assertIn(f"translate zh_hans {identifier}:", text)
        self.assertIn('eve "ZH: We made it."', text)
        self.assertNotIn("\u65e7\u8bd1\u6587", text)
        self.assertIn('new "ZH: Go outside"', text)
        self.assertNotIn("\u65e7\u83dc\u5355", text)
        self.assertEqual(text.count("translate zh_hans strings:"), 1)
        self.assertGreater(
            text.index("translate zh_hans strings:"),
            text.index(f"translate zh_hans {identifier}:"),
        )
        self.assertFalse((merged.parent / "renweave_incremental").exists())
        self.assertEqual([item.relative_path for item in manifest.files], ["game/tl/zh_hans/script.rpy"])

        packaged = TranslationPackager().package(manifest, output / "packages")
        disk_members = {
            f"tl/zh_hans/{path.relative_to(merged.parent).as_posix()}"
            for path in merged.parent.rglob("*.rpy")
        }
        with RpaArchive(packaged.archive_path) as archive:
            self.assertEqual(set(archive.names()), disk_members)

    def test_incremental_dialogue_is_inserted_in_source_order_before_single_strings_block(self) -> None:
        index = ProjectIndexer().build(self.root)
        translations = {unit.id: f"ZH: {unit.source}" for unit in index.text_units}
        emitter = RenpyTranslationEmitter()
        emitter.emit(index, translations, "zh_hans", self.root)
        dialogue_units = [
            unit
            for unit in index.text_units
            if unit.channel in {TextChannel.DIALOGUE, TextChannel.NARRATION}
        ]
        identifiers = emitter.dialogue_identifiers(index)
        missing = dialogue_units[1]
        missing_identifier = identifiers[missing.id]
        script = self.game / "tl" / "zh_hans" / "script.rpy"
        text = script.read_text(encoding="utf-8")
        block = re.compile(
            rf"(?ms)^# [^\n]*:\d+\ntranslate zh_hans {re.escape(missing_identifier)}:\n"
            rf".*?(?=^# [^\n]*:\d+\ntranslate zh_hans |\Z)"
        )
        text, removed = block.subn("", text, count=1)
        self.assertEqual(removed, 1)
        strings_file = self.game / "tl" / "zh_hans" / "strings.rpy"
        strings_text = strings_file.read_text(encoding="utf-8")
        text += "\n" + strings_text[strings_text.index("translate zh_hans strings:"):]
        text += (
            "\ntranslate zh_hans strings:\n\n"
            "    old \"Legacy string\"\n"
            "    new \"保留的旧字符串\"\n"
        ...
        menu = next(unit for unit in index.text_units if unit.source == "Go outside")
        output = Path(self.temp.name) / "source-ordered-incremental-output"
        emitter.emit(
            index,
            translations,
            "zh_hans",
            ...
            existing_language_dir=self.game / "tl" / "zh_hans",
            reused_unit_ids={
                unit.id for unit in index.text_units if unit.id not in {missing.id, menu.id}
            },
        )

        merged = (output / "game" / "tl" / "zh_hans" / "script.rpy").read_text(
            encoding="utf-8"
        )
        headers = [
            match.group(2)
            for line in merged.splitlines()
            if (match := TRANSLATE_HEADER_RE.fullmatch(line))
        ]
        self.assertEqual(
            headers,
            [*(identifiers[unit.id] for unit in dialogue_units), "strings"],
        )
        self.assertEqual(merged.count("translate zh_hans strings:"), 1)
        self.assertIn('new "ZH: Go outside"', merged)
        self.assertIn('new "保留的旧字符串"', merged)

    def test_intermediate_cleanup_rejects_finished_artifacts(self) -> None:
        finished = Path(self.temp.name) / "finished.rpy"
        finished.write_text("translate zh_hans strings:\n", encoding="utf-8")
        with self.assertRaises(ValueError):
            discard_intermediate(finished)
        self.assertTrue(finished.is_file())

        intermediate = Path(self.temp.name) / "_unfinished.tmp"
        intermediate.write_bytes(b"temporary")
        discard_intermediate(intermediate)
    ...
    def test_complete_existing_language_finishes_without_model_calls(self) -> None:
        ...
        workspace = Path(self.temp.name) / "existing-complete-workspace"
        ...
        self.assertTrue(Path(state.output_dir).parents[2].name.startswith("build-"))
        self.assertTrue((workspace / "knowledge.json").is_file())
        self.assertTrue((workspace / "state.json").is_file())
        self.assertTrue((workspace / "translations").is_dir())
        report = json.loads(
    ...
    def test_scope_preview_includes_valid_workspace_checkpoint(self) -> None:
        ...
        workspace = Path(self.temp.name) / "preview-checkpoint-workspace"
        pipeline = RenWeavePipeline(workspace)
        baseline, _budget = pipeline.preview_translation_scope(
            self.root,
            source_language="en",
            target_language="zh_hans",
        )
        self.assertEqual(baseline.model_units, 1)

        scene = next(item for item in index.scenes if item.id == missing_unit.scene_id)
        saved = dict(baseline.translations_by_scene.get(scene.id, {}))
        saved[missing_unit.id] = translations[missing_unit.id]
        pipeline.translations_dir.mkdir(parents=True, exist_ok=True)
        (pipeline.translations_dir / f"{scene.id}.json").write_text(
            json.dumps({"scene_id": scene.id, "translations": saved}),
            encoding="utf-8",
        )

        effective, budget = pipeline.preview_translation_scope(
            self.root,
            source_language="en",
            target_language="zh_hans",
        )

        self.assertEqual(effective.model_units, 0)
        self.assertEqual(effective.workspace_reused_units, 1)
        self.assertEqual(effective.reusable_units, len(index.text_units))
        self.assertEqual(budget.source_token_equivalent, 0)
        self.assertNotIn(missing_unit.id, {item["text_id"] for item in effective.pending_units})
        ...
        class NoCallGateway:
            ...
            def chat(self, _messages, *, temperature=0.2):
                raise AssertionError("A valid workspace checkpoint must not call the model")
        ...
        built = Path(state.output_dir) / "script.rpy"
        built_text = built.read_text(encoding="utf-8")
        self.assertIn(f"translate zh_hans {identifier}:", built_text)
        self.assertIn(f'ZH: {missing_unit.source}', built_text)
    ...
    def test_packager_preserves_prior_rpa_when_content_changes(self) -> None:
        index = ProjectIndexer().build(self.root)
        packages = Path(self.temp.name) / "immutable-packages"
        first_translations = {unit.id: unit.source for unit in index.text_units}
        first_manifest = RenpyTranslationEmitter().emit(
            index, first_translations, "fr", Path(self.temp.name) / "first-rpa-output"
        )
        first = TranslationPackager().package(first_manifest, packages)
        first_path = Path(first.archive_path)
        first_payload = first_path.read_bytes()

        second_translations = dict(first_translations)
        second_translations[index.text_units[0].id] = "Changed translation"
        second_manifest = RenpyTranslationEmitter().emit(
            index, second_translations, "fr", Path(self.temp.name) / "second-rpa-output"
        )
        second = TranslationPackager().package(second_manifest, packages)
