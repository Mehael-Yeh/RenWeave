...
5. 一次启动，持续查看解包、分析、翻译、精修、验证、可选 RPA 打包、ETA 与 Token 用量。
...
它借鉴现代开发工具和编辑型工作区，而不是装饰性的游戏启动器。控件渲染使用随程序打包的 [Sun Valley ttk 主题](https://github.com/rdbende/Sun-Valley-ttk-theme)，语义状态与无障碍规则遵循 Material 3 原则。提供商选择流程的研究参考了 [CC Switch](https://github.com/farion1231/cc-switch)，但织译使用独立的任务型视觉系统与工作流。模型设置始终是第一步：先展示六个常用提供商，其余官方、聚合平台与自定义端点按需展开；所有端点均可编辑。内置预设取代配置文件导入，非密钥 API 设置自动保存。五个页面及弹窗使用同一套视觉与交互语言。关键字段和按钮同时提供行内后果说明与延迟提示，明确用户应填写什么、是否会访问 API、是否可能消耗 Token，以及下一步会改变什么。
