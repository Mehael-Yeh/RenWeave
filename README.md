...
3. **模型**：默认勾选“使用模型翻译”。模型、接口地址和“思考强度”直接展示；可获取并缓存当前提供商的模型列表，也可点击“验证模型”。取消勾选后进入空白翻译路线。
4. **确认**：检查翻译范围、可复用单元、Token 预估和 RPA/安装选项。进入本页只计算工作量，不会开始翻译。
5. **翻译**：点击“开始翻译”或“提取空白翻译”后才会执行工作。运行中可单击“暂停”，暂停完成后按钮变为“继续翻译”，可从检查点恢复；运行中的关闭操作会先询问暂停后退出、立即退出或取消。

进入第 04 页不会调用模型。空白路线不使用模型、不生成 RPA，只生成通过校验的 RPY 翻译文件供用户自行填写。桌面界面和第 05 页运行时状态均支持英文/简体中文即时切换。

界面可以在英文和简体中文之间切换。提供商、接口、模型和思考设置会从用户设置中恢复。API 密钥默认保存到操作系统凭据库，也可以选择只保存在内存中；密钥不会写入织译设置、工作区、日志或打包文件。

桌面界面基于 PySide6 Widgets 构建，五个页面在同一个窗口中持久存在，状态更新只刷新对应模块。这样可以减少切页、滚动、最小化恢复时的重绘闪烁；第 05 页的暂停、恢复、退出确认和实时进度也使用同一套状态机。
...
使用模型翻译时，请填写所选提供商支持的准确模型 ID。03 页始终显示 API 密钥、接口地址、模型和思考强度；模型列表按“提供商 + 接口地址”缓存，切换提供商不会丢失已获取的模型。API 密钥默认隐藏，可用“显示”按钮临时查看。

“继续”有两个放行条件，满足任意一个即可：取消勾选“使用模型翻译”，或验证当前选择的提供商模型成功。切换到未验证的提供商或模型时，“继续”会直接置灰；切回已验证的提供商模型后恢复。思考强度默认为“自动（提供商默认）”。
...
提取空白翻译目前通过桌面流程完成：在 03 页取消勾选“使用模型翻译”，进入 04 页确认后，在 05 页点击“提取空白翻译”。
