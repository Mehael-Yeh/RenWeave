...
3. **Model** — **Use model translation** is enabled by default. API key, endpoint, model, and reasoning level are shown directly. Models can be loaded and cached per provider and endpoint, and the selected model can be verified. Clear the checkbox to use the blank-file route.
4. **Review** — inspect the scope, reusable units, token estimate, and RPA/install options. Entering this page only calculates the workload; it never starts translation.
5. **Translation** — click **Start translation** or **Extract blank translation** to begin. While running, one click on **Pause** requests a safe pause; after checkpoints are saved, the button becomes **Resume translation**. Closing a running task asks whether to pause then exit, exit immediately, or cancel.

Entering Review never starts a model request. The blank route does not call a model or create an RPA; it generates validated RPY translation files for manual completion. The desktop UI, including live step-05 status, can switch between English and Simplified Chinese without rebuilding the pages.
...
The desktop UI uses PySide6 Widgets. All five pages remain mounted in one window and workers update only the affected modules, reducing redraw flicker during navigation, scrolling, and minimize/restore. Step 05 uses the same state machine for live progress, safe pause/resume, and exit confirmation.
...
Enter the exact model ID supported by the selected provider. Step 03 always shows the API key, endpoint, model, and reasoning level; there is no separate Advanced Settings page. Model catalogs are cached per provider and endpoint, and API keys remain masked unless the user presses **Show**.

Continue is enabled when either model translation is disabled or the currently selected provider/model has been verified successfully. Switching to an unverified provider or model disables Continue immediately; switching back to a verified provider/model restores it. Reasoning defaults to **Automatic (provider default)**. The first translation request occurs only after the explicit action on step 05.
...
Blank translation extraction is currently a desktop workflow: clear **Use model translation** on step 03, confirm the scope on step 04, then choose **Extract blank translation** on step 05.
